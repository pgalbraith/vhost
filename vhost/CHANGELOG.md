...
### Added
- [[#PR]] Add a Windows vhost-user backend transport, naming Win32 kernel objects in the message
  payload in place of `SCM_RIGHTS` descriptor passing.

### Changed
- [[#PR]] Split `connection.rs`'s POSIX-specific code into `connection/unix.rs`; gate the
  vhost-user frontend, GPU channel, and backend request channel to unix.

### Deprecated
### Fixed
- [[#PR]] Don't enable vm-memory's `rawfd` feature on Windows, where it's a hard compile error.
